...
///
/// This model provides details about a given bearer token, including associated
/// user information (if applicable), scopes, authorities, expiration time, and client ID.
/// In the case of a client credentials flow token, [userName] will be `null`.
@immutable
class CheckTokenResponse {
  ...
  Map<String, dynamic> toJson() => <String, dynamic>{
    ...
    'account_authorities': List<dynamic>.from(
      accountAuthorities.map((String x) => x),
    ),
    'exp': exp,
    'client_authorities': List<dynamic>.from(
      clientAuthorities.map((String x) => x),
    ),
  ...
  bool operator ==(Object other) =>
      ...
      other is CheckTokenResponse &&
          ...
          const DeepCollectionEquality().equals(
            accountAuthorities,
            other.accountAuthorities,
          ) &&
          exp == other.exp &&
          const DeepCollectionEquality().equals(
            clientAuthorities,
            other.clientAuthorities,
          ) &&
          const DeepCollectionEquality().equals(
            authorities,
            other.authorities,
