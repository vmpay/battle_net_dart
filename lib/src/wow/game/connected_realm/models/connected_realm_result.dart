...
///
/// This model encapsulates the key information and associated data for a connected realm.
@immutable
class ConnectedRealmResult {
  ...
  const ConnectedRealmResult({required this.key, required this.data});

  /// Returns a copy of this instance with the given fields replaced
  /// by new values.
  ///
  /// If a field is not provided, the existing value is retained.
  ConnectedRealmResult copyWith({Self? key, ConnectedRealmSearchData? data}) =>
      ConnectedRealmResult(key: key ?? this.key, data: data ?? this.data);
